...
## 📌 1. `grok-proxy.js` (v4.5 — Interactive Overseer Console & Smart Balancer)

Локальный прокси для Grok API (`grok-4.5` / `tunnel.rue.onl`) с веб-панелью управления и динамическими инъекциями.

### Ключевые возможности:
- **Интерактивная Веб-Панель (`http://127.0.0.1:8319/`):** интерфейс на Glassmorphic CSS с кнопками быстрых пресетов, живой консолью инъекций и стримингом логов в реальном времени.
- **Классификатор Ролей (Subagent Role Classifier):** автоматическое дифференцирование Главных агентов и Сабагентов. Сабагенты получают быстрый поисковый ошейник (`SUBAGENT GUARD`), а Главные агенты — стратегические цели.
- **Bulletproof Directive Injector:** подмешивание архитектурных директив прямо в стрим сообщений без нарушения правила чередования ролей (Role Alternation).
- **Strict User-Protected 413 Trimmer:** реактивное усечение длинных дамп-выводов консоли при ошибке `HTTP 413 Payload Too Large` без потери пользовательского промпта.
- **Умный балансировщик (Smart LRU Balancer):** авто-выравнивание нагрузки между API-ключами, Dead Key Guard (401/403) и 20-секундный паузер при `429 Rate Limit`.

### Быстрый запуск:
1. Для собственного запуска создай `keys.json` с ключами или используй `grok-proxy-EXPORT.js`.
2. Запусти `Grok Proxy.cmd` (или `node grok-proxy.js`).
3. Открой `http://127.0.0.1:8319/` для доступа к интерактивной панели управления.
