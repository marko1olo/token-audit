...
function checkAndInjectDirectives(bodyBuffer, sessionId, keyIdx) {
  ...
  try {
    ...
    if (injections.all && typeof injections.all === 'string') {
      ...
      const sLower = String(sessionId).toLowerCase();
      for (const [key, dir] of Object.entries(injections)) {
        const kLower = String(key).toLowerCase();
        if (dir && typeof dir === 'string' && (sLower.includes(kLower) || kLower.includes(sLower))) {
          targetDirective = dir;
  ...
  req.on('end', () => {
    ...
    if (body.length > 0) {
      try {
        ...
        if (!sessionId && Array.isArray(obj.messages)) {
          // Frequency-based project detection: immune to prompt cross-mentions
          const bodyStr = JSON.stringify(obj.messages).toLowerCase();
          const counts = {
            'Gigahrush2': (bodyStr.match(/gigahrush2|gigahrush/g) || []).length,
            'Hecton8': (bodyStr.match(/hecton8|hectonplayer/g) || []).length,
            'Dental CRM': (bodyStr.match(/dental-crm|clinic_mvp|dental/g) || []).length,
          };
          let best = 'Gigahrush2';
          for (const k of ['Hecton8', 'Dental CRM']) {
