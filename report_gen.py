...
def _ig(c):
    ...
    ok = True
    # Допуск здесь НУЛЕВОЙ, и это принципиально. Раньше стояло 2%, и под этим
    # допуском полгода жило расхождение 0.19%: claude_agg и claude_deep --
    # два процесса, сканирующие растущий каталог с разницей около минуты, и
    # второй видел записи, которых первый не видел. Итог печатался из одного
    # артефакта, таблица по моделям и деньги -- из другого, доли по моделям
    # суммировались в 100.19%, а дашборд показывал две разные итоговые суммы
    # на одной странице ($8 712,39 и $8 737,27). Два измерения одних и тех же
    # записей обязаны совпадать до токена; допуск тут скрывает дефект, а не
    # поглощает шум. Общее окно сканирования обеспечивает tokenaudit_scan.py.
    s = sum(v["total"] for v in c.dp["by_model"].values())
    diff = abs(s - c.total)
    good = diff == 0
    ok &= good
    rows.append("| сумма по моделям против итога | %s%s |" % (
        "СХОДИТСЯ" if good else "РАСХОДИТСЯ",
        "" if good else " (расхождение %s токенов -- окна сканирования разошлись)" % f(diff)))
    # Число ответов -- вторая, независимая от сумм, проверка того же самого.
    ra = c.cl.get("records_deduped")
    rd = c.dp.get("responses")
